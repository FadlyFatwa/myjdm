...
class Ap_kontra_bon_payment_m extends CI_Model {
    ...
    public function payment_no()
    {
        $sql = "SELECT
                    ...
                    MAX(MID(payment_no, 8, 4)) AS last_no
                FROM ap_kontra_bon_payment
                WHERE MID(payment_no, 4, 4) = DATE_FORMAT(CURDATE(), '%y%m')";
    ...
    {
        $this->db->select('ap_kontra_bon_payment.*, user.nama AS paid_by_name');
        $this->db->from('ap_kontra_bon_payment');
        $this->db->join('user', 'user.user_id = ap_kontra_bon_payment.paid_by');
        $this->db->where('ap_kontra_bon_payment.kontra_bon_id', $kontra_bon_id);
        $this->db->order_by('ap_kontra_bon_payment.kontra_bon_payment_id', 'DESC');
        return $this->db->get()->result();
    }

    public function get($id)
    {
        return $this->db->where('kontra_bon_payment_id', $id)->get('ap_kontra_bon_payment')->row();
    }

    public function get_detail($kontra_bon_payment_id)
    {
        $this->db->select('ap_kontra_bon_payment_detail.*, ap_invoice.ap_no');
        $this->db->from('ap_kontra_bon_payment_detail');
        $this->db->join('ap_invoice', 'ap_invoice.ap_invoice_id = ap_kontra_bon_payment_detail.ap_invoice_id');
        $this->db->where('ap_kontra_bon_payment_detail.kontra_bon_payment_id', $kontra_bon_payment_id);
        return $this->db->get()->result();
    }
    ...
    {
        $kb = $this->Ap_kontra_bon_m->get($post['kontra_bon_id']);
        ...
        $locked = $this->db->query(
            "SELECT outstanding_amount, status FROM ap_kontra_bon WHERE kontra_bon_id = ? FOR UPDATE",
            [$post['kontra_bon_id']]
        )->row();
        ...
        $journal_id = $this->Journal_m->post([
            ...
            'source_type'  => 'ap_kontra_bon_payment',
            'source_id'    => null,
            'description'  => 'Pembayaran kontra bon ' . $kb->kontra_bon_no,
            'created_by'   => $user_id,
        ], [
        ...
        $this->db->insert('ap_kontra_bon_payment', [
        ...
        foreach ($invoices as $inv) {
            ...
            $this->db->insert('ap_kontra_bon_payment_detail', [
                'kontra_bon_payment_id' => $kontra_bon_payment_id,
                'ap_invoice_id'         => $inv->ap_invoice_id,
                'amount_allocated'      => $alloc,
            ]);

            $this->Ap_invoice_m->update_paid_amount($inv->ap_invoice_id, $alloc);
            $remaining -= $alloc;
        }

        $this->Ap_kontra_bon_m->update_paid_amount($post['kontra_bon_id'], $amount);
    ...
    public function void_payment($id, $reason, $user_id)
    {
        ...
        foreach ($details as $d) {
            $this->Ap_invoice_m->update_paid_amount($d->ap_invoice_id, -$d->amount_allocated);
        }

        $this->Ap_kontra_bon_m->update_paid_amount($payment->kontra_bon_id, -$payment->amount);

        $this->db->where('kontra_bon_payment_id', $id)->update('ap_kontra_bon_payment', [
            'is_void'     => 1,
            'void_reason' => $reason,
            'voided_by'   => $user_id,
