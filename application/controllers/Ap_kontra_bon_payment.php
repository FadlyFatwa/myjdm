...
class Ap_kontra_bon_payment extends CI_Controller {
    ...
    {
        $kb = $this->Ap_kontra_bon_m->get($kontra_bon_id);
        if (!$kb) show_404();
        ...
        $data['kb']    = $kb;
        $this->template->load('template', 'finance/ap_kontra_bon_payment/ap_kontra_bon_payment_form', $data);
    }
    ...
    public function process()
    {
        ...
        try {
            $this->Ap_kontra_bon_payment_m->pay($post, $this->fungsi->user_login()->user_id);
            $this->session->set_flashdata('success', 'Pembayaran kontra bon berhasil dicatat.');
        } catch (Exception $e) {
            $this->session->set_flashdata('error', $e->getMessage());
            redirect('ap-kontra-bon-payment/add/' . $post['kontra_bon_id']);
        }
        redirect('ap-kontra-bon/detail/' . $post['kontra_bon_id']);
    }
    ...
    public function void($id)
    {
        ...
        $payment = $this->Ap_kontra_bon_payment_m->get($id);
        if (!$payment) show_404();

        try {
            $this->Ap_kontra_bon_payment_m->void_payment($id, $this->input->post('void_reason', TRUE), $this->fungsi->user_login()->user_id);
            $this->session->set_flashdata('success', 'Pembayaran berhasil di-void.');
        } catch (Exception $e) {
            $this->session->set_flashdata('error', $e->getMessage());
        }
        redirect('ap-kontra-bon/detail/' . $payment->kontra_bon_id);
    }
}
